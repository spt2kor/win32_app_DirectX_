...
HRESULT DemoApp::OnRender()
{
	...
	HRESULT hr = CoCreateInstance(CLSID_WICImagingFactory, NULL, CLSCTX_LOCAL_SERVER, IID_PPV_ARGS(&pWIC));

	PCWSTR resourceName = L"C:\\Program Files (x86)\\Microsoft DirectX SDK (June 2010)\\Samples\\Media\\misc\\lake.bmp";
	ID2D1Bitmap *ppBitmap = NULL;
	LoadResourceBitmap(m_pRenderTarget, pWIC, resourceName, L"Image", 200, 100, &ppBitmap);
	...
	)
{
	...
	/*
	Before Direct2D can use the image, it must be converted to the 32bppPBGRA pixel format. To convert the image format,
	use the IWICImagingFactory::CreateFormatConverter method to create an IWICFormatConverter object, 
	then use the IWICFormatConverter object's Initialize method to perform the conversion.
