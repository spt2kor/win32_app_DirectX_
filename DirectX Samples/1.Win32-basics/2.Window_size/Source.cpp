                   ...
                   int nCmdShow) // controls how the window is to be shown
{
    ...
    RECT wr = {0, 0, 500, 400};    // RECT structure that set the size, but not the position
    AdjustWindowRect(&wr, WS_OVERLAPPEDWINDOW, FALSE); //Calculates the required size of the window rectangle, based on the desired client-rectangle size.
    ...
    hWnd = CreateWindowEx(NULL,
                          ...
                          wr.right - wr.left,    // width of the window //width will be the difference between the right and the left
                          wr.bottom - wr.top,    // height of the window // height will be the difference between the bottom and the top.
	...
	//while (PeekMessage (&msg, NULL, 0, 0, PM_REMOVE)) // check the message queue for messages , if no message it will return nothing
    {
        ...
        case WM_DESTROY:  
            {
				...
				MessageBox ( NULL , L"End of Window_size",L"Thank you !!", MB_ICONSTOP | MB_OK ); // MB_ICONWARNING
